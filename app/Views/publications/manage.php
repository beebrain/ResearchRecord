        ...
        <main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            ...
            <div class="bg-white rounded-lg shadow p-6 mb-6">
                <div class="flex flex-col sm:flex-row gap-4 justify-between">
                    ...
                    <a href="<?= site_url('publications/create?return=' . rawurlencode('/publications/manage')) ?>" class="px-6 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-lg font-medium transition-colors inline-flex items-center justify-center gap-2">
                    ...
                    <table class="w-full">
                        <thead class="bg-gray-50 border-b border-gray-200">
                            <tr>
                                ...
                                <th class="px-6 py-4 text-left text-sm font-semibold text-gray-700">สถานะอนุมัติ</th>
        ...
        <script>
            ...
            async function loadPublications() {
                try {
                    ...
                    // For Faculty Admin, API will automatically filter by their managed faculties
                    // For Super Admin, API will return all publications
                    const currentUserId = <?= json_encode($user['email'] ?? '') ?>;
                    const userRole = '<?= isset($user['role']) ? $user['role'] : 'user' ?>';
                    const isAdmin = <?= (isset($is_admin) && $is_admin) ? 'true' : 'false' ?>;

                    // IMPORTANT: Only add filter_user_id for regular users (not admin)
                    // - Regular users: Filter by their own user ID (shows only their publications)
                    // - Faculty Admin: API will automatically filter by their managed faculties/curricula
                    // - Super Admin: API will return all publications
                    let apiUrl = '<?= site_url('api/dashboard/publications') ?>?limit=1000';

                    // Only regular users (not admin, not faculty_admin, not super_admin) should use filter_user_id
                    if (userRole === 'user' && !isAdmin) {
                        apiUrl += '&filter_user_id=' + currentUserId;
                        console.log('Regular user detected - filtering by user ID');
                    } else if (userRole === 'faculty_admin') {
                        console.log('Faculty Admin detected - API will filter by managed faculties/curricula');
                    } else if (userRole === 'super_admin' || isAdmin) {
                        console.log('Super Admin detected - API will return all publications');
                    }
                    ...
                    console.log('Will filter by user ID:', (userRole === 'user' && !isAdmin));
                    ...
                    if (result.success) {
                        ...
                        console.log('Note: For Faculty Admin, only publications from managed faculties/curricula are shown');
            ...
            function renderTable() {
                ...
                tbody.innerHTML = filteredPublications.map((pub) => {
                    ...
                    <tr class="border-t border-gray-200 hover:bg-gray-50">
                        ...
                        <td class="px-6 py-4">
                            ${getApprovalStatusBadge(pub.approve)}
                        </td>
                        <td class="px-6 py-4">
            ...
            async function editPublication(id) {
                try {
                    ...
                    const apiUrl = BASE_URL + '/index.php/publications/get/' + id;
                    const response = await fetch(apiUrl);
                    const result = await response.json();
            ...
            async function deletePublication(id, title) {
                ...
                if (result.isConfirmed) {
                    try {
                        ...
                        const apiUrl = BASE_URL + '/index.php/publications/delete/' + id;
                        const response = await fetch(apiUrl, {
                            method: 'DELETE',
                            headers: {
            ...
            document.getElementById('editForm').addEventListener('submit', async (e) => {
                ...
                try {
                    ...
                    const response = await fetch(BASE_URL + '/index.php/publications/update/' + publicationId, {
                        method: 'POST',
                        body: formData
                    });
